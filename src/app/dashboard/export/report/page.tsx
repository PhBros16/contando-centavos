...
export default async function ReportPage({
  ...
  const [{ data: profile }, { data: transactions }, { data: goals }, { data: accounts }, { data: allAccountTx }] =
    await Promise.all([
  ...
  for (const acc of accounts ?? []) accountBalances[acc.id] = Number(acc.initial_balance);
  for (const t of allAccountTx ?? []) {
    if (accountBalances[t.account_id] !== undefined) accountBalances[t.account_id] += Number(t.amount);
  }
